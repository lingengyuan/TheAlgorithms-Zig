...
test "collatz steps: known values" {
    try testing.expectEqual(@as(u64, 0), try collatzSteps(1));
    try testing.expectEqual(@as(u64, 1), try collatzSteps(2));
    try testing.expectEqual(@as(u64, 2), try collatzSteps(4));
    try testing.expectEqual(@as(u64, 14), try collatzSteps(11));
}

test "collatz sequence: n=4" {
    var buf: [100]u64 = undefined;
    const seq = try collatzSequence(4, &buf);
    try testing.expectEqualSlices(u64, &[_]u64{ 4, 2, 1 }, seq);
}

test "collatz sequence: n=11" {
    var buf: [100]u64 = undefined;
    const seq = try collatzSequence(11, &buf);
...
test "collatz sequence: n=1" {
    var buf: [100]u64 = undefined;
    const seq = try collatzSequence(1, &buf);
    try testing.expectEqualSlices(u64, &[_]u64{1}, seq);
}

test "collatz: invalid input" {
    try testing.expectError(error.InvalidInput, collatzSteps(0));
    var buf: [4]u64 = undefined;
    try testing.expectError(error.InvalidInput, collatzSequence(0, &buf));
}

test "collatz: buffer too small" {
    var buf: [2]u64 = undefined;
    try testing.expectError(error.BufferTooSmall, collatzSequence(4, &buf));
}
