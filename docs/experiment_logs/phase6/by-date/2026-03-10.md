...
- `zig test graphs/check_bipartite.zig` ✅
...
## Phase 6 Full-Test Observation / 第 6 阶段全量测试观察 (2026-03-10)

Scope / 范围:
- `zig build test`
- `ps -eo pid,ppid,etime,pcpu,pmem,cmd | rg 'zig build test|/test --seed'`
...
## Phase 6 Batch E - Wave 24 / 第 6 阶段 Batch E - 第 24 波 (2026-03-10)

Scope / 范围:
- `project_euler/problem_068.zig`
- `project_euler/problem_070.zig`
- `project_euler/problem_078.zig`
- `project_euler/problem_086.zig`
- `build.zig`
- `README.md`
- `EXPERIMENT_LOG.md`
- `docs/experiment_logs/phase6/by-date/2026-03-10.md`
...
Result / 结果:
- Added four more `Project Euler` implementations: `068`, `070`, `078`, and `086`.
- 已新增 4 个 `Project Euler` 实现：`068`、`070`、`078`、`086`。
- Updated repository accounting to `906` registered algorithms, `898` effective completed algorithms, and `18` remaining planned algorithms under the corrected portable target of `916`.
- 已将仓库统计更新为：在修正后的 `916` 个可移植目标口径下，当前共有 `906` 个已注册算法、`898` 个有效完成算法、`18` 个剩余计划缺口。

Verification / 验证:
- `zig test project_euler/problem_068.zig` ✅
- `zig test project_euler/problem_070.zig` ✅
- `zig test project_euler/problem_078.zig` ✅
...
Scope / 范围:
- `project_euler/problem_119.zig`
- `project_euler/problem_129.zig`
- `project_euler/problem_135.zig`
- `project_euler/problem_136.zig`

Result / 结果:
- Added four more `Project Euler` implementations: `119`, `129`, `135`, and `136`.
- 已新增 4 个 `Project Euler` 实现：`119`、`129`、`135`、`136`。
- Deferred repository-level accounting until the final registration wave because these files were still outside `build.zig` at this point.
- 因这些文件当时尚未接入 `build.zig`，本波暂不推进仓库级总量统计，统一留到最终注册波次收口。
...
Scope / 范围:
- `project_euler/problem_080.zig`
- `project_euler/problem_122.zig`
- `project_euler/problem_164.zig`
- `project_euler/problem_187.zig`
...
Scope / 范围:
- `project_euler/problem_101.zig`
- `project_euler/problem_345.zig`
- `project_euler/problem_493.zig`
- `project_euler/problem_587.zig`
...
Scope / 范围:
- `project_euler/problem_104.zig`
- `project_euler/problem_144.zig`
- `project_euler/problem_234.zig`
- `project_euler/problem_686.zig`
- `project_euler/problem_800.zig`
...
Scope / 范围:
- `project_euler/problem_180.zig`
- `project_euler/problem_551.zig`
- `build.zig`
- `README.md`
- `EXPERIMENT_LOG.md`
- `docs/experiment_logs/phase6/by-date/2026-03-10.md`
...
- Updated repository accounting to `925` registered algorithms, `916` effective completed algorithms, and `0` remaining planned algorithms under the corrected portable target of `916`.
- 已将仓库统计更新为：在修正后的 `916` 个可移植目标口径下，当前共有 `925` 个已注册算法、`916` 个有效完成算法、`0` 个剩余计划缺口。
...
Scope / 范围:
- `zig build test`
- `ps -eo pid,ppid,etime,state,pcpu,pmem,cmd | rg '/test --seed|zig build test|/build /root/zig/zig'`
...
## Post-Phase Review Remediation - Critical Batch 1 / 阶段收口后复核修复 - 严重问题第 1 批 (2026-03-10)
...
Result / 结果:
- Verified the first `9` critical items from `REVIEW_FINDINGS.md` against source code before modifying anything.
- 在修改前先对照源码核验了 `REVIEW_FINDINGS.md` 中第一批 `9` 个严重问题。
- Confirmed these were real logic defects, not just naming disagreements or review noise.
- 确认这些都是实际逻辑缺陷，而不是命名分歧或评审噪音。
- Fixed the affected algorithms and rewrote the relevant tests so they now verify correctness instead of preserving buggy outputs.
- 已修复相关算法，并重写对应测试，使其验证真实正确性，而不是把错误输出固化为期望值。
...
- Failing Step/Command / 失败步骤或命令:
  - `zig test matrix/inverse_of_matrix.zig`
  - Symptom / 现象: after fixing the 3x3 adjugate placement, my first rewritten test still failed because one expected inverse entry had the wrong sign.
  - 中文说明：修正 3x3 伴随矩阵位置后，我第一次重写的测试仍然失败，因为有一个期望逆矩阵元素的符号写错了。
  - Root Cause / 根因: I corrected the implementation first but transcribed one expected matrix value incorrectly during the test rewrite.
  - 中文说明：我先修了实现，但在重写测试时抄错了一个期望矩阵值。
  - Fix Applied / 修复措施: corrected the expected value and added an explicit `A * A^{-1} = I` identity check.
  - 中文说明：已修正期望值，并补充显式的 `A * A^{-1} = I` 单位阵校验。
  - Post-Fix Verification / 修复后验证: `zig test matrix/inverse_of_matrix.zig` passed.
  - 中文说明：修复后 `zig test matrix/inverse_of_matrix.zig` 已通过。
- Failing Step/Command / 失败步骤或命令:
  - `zig test data_structures/quadratic_probing.zig`
  - Symptom / 现象: once collision resolution was corrected to preserve displaced values, one old doctest expectation failed because it had been accepting the previous overwrite bug and dropped value `0`.
  - 中文说明：当碰撞解析修正为保留已位移值后，一个旧 doctest 期望失败了，因为它实际上接受了之前的覆盖 bug，把值 `0` 丢掉了。
  - Root Cause / 根因: the previous test suite had encoded buggy overwrite behavior as the expected outcome.
  - 中文说明：旧测试把错误的覆盖行为编码成了期望结果。
  - Fix Applied / 修复措施: updated the legacy expectation and added a dedicated regression case where a displaced value equals the original bucket index.
  - 中文说明：已更新旧期望，并补了一个“位移值恰好等于原桶索引”的专门回归用例。
  - Post-Fix Verification / 修复后验证: `zig test data_structures/quadratic_probing.zig` passed.
  - 中文说明：修复后 `zig test data_structures/quadratic_probing.zig` 已通过。
- Failing Step/Command / 失败步骤或命令:
  - `zig test boolean_algebra/quine_mc_cluskey.zig`
  - Symptom / 现象: after replacing the inverted merge-marking logic with real term merging, the first attempt crashed with allocator overflow/panic while transferring next-iteration terms.
  - 中文说明：把反向合并标记逻辑改成真正的项合并后，第一次实现在线程下一轮项集转移时触发了分配器溢出/崩溃。
  - Root Cause / 根因: I freed the old `current` list and then reused the deinitialized container instead of transferring ownership cleanly from the next-terms list.
  - 中文说明：我释放了旧 `current` 列表后，又错误复用了已失效的容器，而不是从下一轮列表干净地转移所有权。
  - Fix Applied / 修复措施: transferred ownership from `next_terms` to `current` directly and updated the test to assert the real merged implicant `00_`.
  - 中文说明：改为直接把 `next_terms` 的所有权转移给 `current`，并把测试更新为断言真实合并项 `00_`。
  - Post-Fix Verification / 修复后验证: `zig test boolean_algebra/quine_mc_cluskey.zig` passed.
  - 中文说明：修复后 `zig test boolean_algebra/quine_mc_cluskey.zig` 已通过。

## Post-Phase Review Remediation - Major Batch 1 / 阶段收口后复核修复 - 重大问题第 1 批 (2026-03-10)
...
Result / 结果:
- Fixed a first batch of `12` high-confidence major issues from `REVIEW_FINDINGS.md`.
- 已修复 `REVIEW_FINDINGS.md` 中第一批 `12` 个高确定性的重大问题。
- Replaced reference-compatible-but-wrong expectations with semantic checks aligned to the actual algorithms.
- 已将“与旧错误行为兼容”的期望替换为与真实算法语义一致的校验。
...
- Failing Step/Command / 失败步骤或命令:
  - `zig test strings/frequency_finder.zig`
  - Symptom / 现象: after fixing ETAOIN tie-breaking, the old `englishFreqMatchScore("Hello World")` expectation failed because it had been derived from the reversed ordering bug.
  - 中文说明：修正 ETAOIN 并列频次排序后，旧的 `englishFreqMatchScore("Hello World")` 期望值失败了，因为它来自之前反向排序的错误行为。
  - Root Cause / 根因: the regression test had encoded the buggy ordering, so the match score itself also became stale.
  - 中文说明：回归测试把错误排序编码进去了，因此匹配分数也一起过期了。
  - Fix Applied / 修复措施: updated the score expectation to the corrected ordering result and added an empty-input frequency-order check.
  - 中文说明：已把分数期望更新为修正后排序对应的结果，并补充空输入的频率排序检查。
  - Post-Fix Verification / 修复后验证: `zig test strings/frequency_finder.zig` passed.
  - 中文说明：修复后 `zig test strings/frequency_finder.zig` 已通过。

## Post-Phase Review Remediation - Major Batch 2 / 阶段收口后复核修复 - 重大问题第 2 批 (2026-03-10)
...
Result / 结果:
- Fixed another `11` major issues covering leak-safe error cleanup, overflow guards, scheduling correctness, and exact-fit formatting.
- 已修复另外 `11` 个重大问题，涵盖泄漏安全的错误清理、溢出保护、调度正确性以及精确贴合宽度时的格式化问题。
...
- `zig test maths/binomial_distribution.zig` ✅
- `zig test maths/lucas_lehmer_primality_test.zig` ✅
- `zig test maths/qr_decomposition.zig` ✅
...
- Failing Step/Command / 失败步骤或命令:
  - `zig test data_structures/serialize_deserialize_binary_tree.zig`
  - Symptom / 现象: the first allocation-failure regression still reported leaks because the test cleanup path was calling the regular `freeTree()`, which itself allocates a temporary stack and can fail under a failing allocator.
  - 中文说明：第一版分配失败回归测试仍报告泄漏，因为测试清理路径调用了常规 `freeTree()`，而它内部会分配临时栈，在 failing allocator 下也可能失败。
  - Root Cause / 根因: I had fixed the build-time error path with a no-allocation cleanup, but the allocation-failure test itself was still using the allocating cleanup routine.
  - 中文说明：我只修了构建阶段的无分配清理路径，但分配失败测试本身仍在使用会分配内存的清理函数。
  - Fix Applied / 修复措施: added `freeTreeNoAlloc()` and used it both in `buildFromTokens()` error cleanup and in the allocation-failure regression helper.
  - 中文说明：新增 `freeTreeNoAlloc()`，并在 `buildFromTokens()` 的错误清理以及分配失败回归辅助函数中统一使用。
  - Post-Fix Verification / 修复后验证: `zig test data_structures/serialize_deserialize_binary_tree.zig` passed.
  - 中文说明：修复后 `zig test data_structures/serialize_deserialize_binary_tree.zig` 已通过。
- Failing Step/Command / 失败步骤或命令:
  - `zig test searches/tabu_search.zig`
  - Symptom / 现象: the new allocation-failure regression initially found a leak in `findNeighborhood()` even after the best-solution double-free fix was in place.
  - 中文说明：即便修掉了 best-solution 双重释放，这个新的分配失败回归测试仍在 `findNeighborhood()` 里发现了一处泄漏。
  - Root Cause / 根因: when `allocator.dupe()` for a swapped route succeeded but `candidates.append()` failed, the temporary `swapped` buffer had no local cleanup.
  - 中文说明：当交换后的路径 `allocator.dupe()` 成功、但 `candidates.append()` 失败时，临时 `swapped` 缓冲区没有局部清理逻辑。
  - Fix Applied / 修复措施: added a local `errdefer allocator.free(swapped)` before appending the candidate, and kept the improved replacement ordering for `best_solution`.
  - 中文说明：在追加候选前增加了局部 `errdefer allocator.free(swapped)`，并保留了 `best_solution` 替换顺序修复。
  - Post-Fix Verification / 修复后验证: `zig test searches/tabu_search.zig` passed.
  - 中文说明：修复后 `zig test searches/tabu_search.zig` 已通过。

## Post-Phase Review Remediation - Minor Batch 1 / 阶段收口后复核修复 - 次要问题第 1 批 (2026-03-10)
...
Result / 结果:
- Fixed `4` low-risk issues involving integer underflow, unnecessary duplicate stack pushes, `mod 1` semantics, and allocation-failure cleanup.
- 已修复 `4` 个低风险问题，涉及整数下溢、重复入栈、`mod 1` 语义以及分配失败清理。

Verification / 验证:
- `zig test data_structures/treap.zig` ✅
- `zig test graphs/depth_first_search_2.zig` ✅
- `zig test maths/binary_exponentiation.zig` ✅
- `zig test strings/ngram.zig` ✅

## Post-Phase Review Verification - Remaining Graph Claims / 阶段收口后复核核验 - 剩余图搜索项 (2026-03-10)
...
Result / 结果:
- Constructed an explicit counterexample graph showing that both bidirectional BFS variants could return a non-shortest path because they alternated one node per side instead of expanding full frontier levels.
- 已构造出明确反例，证明两个双向 BFS 变体都会因为“每轮每侧只扩展一个节点”而返回非最短路径。
- Fixed both implementations to expand one complete BFS level per side per iteration, and added regression tests for the counterexample.
- 已将两个实现修正为“每轮每侧扩展完整一层 BFS frontier”，并为该反例增加回归测试。
- Verified that the `a_star_search` report item is a false positive under the current API contract, because the function explicitly rejects inconsistent heuristics before search begins.
- 已确认 `a_star_search` 这条评审项在当前 API 约束下属于误报，因为该函数在搜索开始前就显式拒绝不一致启发式。
...
- All actionable defects from `REVIEW_FINDINGS.md` were fixed in code and covered by file-level regressions plus a final full-suite pass.
- `REVIEW_FINDINGS.md` 中所有可执行缺陷都已完成代码修复，并由文件级回归测试和最终全量测试共同覆盖。
- The `graphs/a_star_search.zig` reopen claim was kept as a false positive under the current API contract because inconsistent heuristics are rejected before search.
- `graphs/a_star_search.zig` 的 reopen 指控在当前 API 合同下保留为误报，因为不一致启发式会在搜索前被拒绝。
- The remaining `INFO` item for `conversions/octal_to_hexadecimal.zig` was intentionally retained as a documented Python-compatibility quirk rather than changed.
- `conversions/octal_to_hexadecimal.zig` 的剩余 `INFO` 项被有意保留为已文档化的 Python 兼容性特殊行为，而不是继续修改代码。
